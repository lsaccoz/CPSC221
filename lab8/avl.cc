...
void balance(Node *& x) {
    ...
    int height_left = !(x->left) ? -1 : x->left->height;
    int height_right = !(x->right) ? -1 : x->right->height;

    //Unbalanced to the right
    if (height_right - height_left > 1) {
        if (x->right->right && (height_right == x->right->right->height + 1)) {
            // Right-Right
            rotateLeft(x);
        } else {
            // Right-Left
    ...
    //Unbalanced to the left
        ...
        if (x->left->left && (height_left == x->left->left->height + 1)) {
            // Left-Left
            rotateRight(x);
        } else {
            // Left-Right
